...
add_filter( 'comment_form_default_fields', 'aliaala01sttheme_bootstrap_comment_form_fields' );

if ( ! function_exists( 'aliaala01sttheme_bootstrap_comment_form_fields' ) ) {
	/**
	 ...
	 * @return array
	 */
	function aliaala01sttheme_bootstrap_comment_form_fields( $fields ) {
...
add_filter( 'comment_form_defaults', 'aliaala01sttheme_bootstrap_comment_form' );

if ( ! function_exists( 'aliaala01sttheme_bootstrap_comment_form' ) ) {
	/**
	 ...
	 * @return string[]
	 */
	function aliaala01sttheme_bootstrap_comment_form( $args ) {
		$replace = array(
			'<p class="' => '<div class="form-group mb-3 ',
...
} // End of if function_exists( 'aliaala01sttheme_bootstrap_comment_form' ).


// Add note if comments are closed.
add_action( 'comment_form_comments_closed', 'aliaala01sttheme_comment_form_comments_closed' );

if ( ! function_exists( 'aliaala01sttheme_comment_form_comments_closed' ) ) {
	/**
	 * Displays a note that comments are closed if comments are closed and there are comments.
	 */
	function aliaala01sttheme_comment_form_comments_closed() {
		if ( get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) {
			?>
			<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'aliaala01sttheme' ); ?></p>
			<?php
		}
	}
} // End of if function_exists( 'aliaala01sttheme_comment_form_comments_closed' ).
